...
public class Media {
	...
	public Situacao obterSituacao() {
		...
		if (media >= Situacao.APROVADO.notaCorte)
			return Situacao.APROVADO;
		else if (media >= Situacao.RECUPERAÇÃO.notaCorte)
	...
	enum Situacao {
		APROVADO(7, "Você foi aprovado"), RECUPERAÇÃO(5, "você esta de recuperação"),
		REPROVADO(0, "você foi reprovado");

		private final int notaCorte;
		...
		Situacao(int notaCorte, String texto) {
	...
	private void adicionarNota(double nota) {
		...
		if (Double.isNaN(nota1))
			nota1 = nota;
		else if (Double.isNaN(nota2))
			nota2 = nota;
		else if (Double.isNaN(nota3))
			nota3 = nota;
		else if (Double.isNaN(nota4))
			nota4 = nota;
		else {
	...
	public static boolean entrarComDigito(Media media) {
		...
		try {
			...
			deuErro = true;
			System.out.println(
					"Por favor, certifique-se de digitar apenas valores numéricos. Caso haja casas decimais no valor digitado, utilize a vírgula (,) em vez do ponto (.) como separador decimal.");
	...
	public static void main(String[] args) {
		...
		Situacao situacao = media.obterSituacao();

		sb.append("\n");
		sb.append(situacao.getTexto());
